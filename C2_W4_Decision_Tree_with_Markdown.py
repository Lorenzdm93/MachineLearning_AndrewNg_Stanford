...
def compute_entropy(y):
    ...
    ### START CODE HERE ###
    if len(y) != 0:   #if data at first node is not emtpy then calculate entropy otherwise proceed
        p1 = p1 = len(y[y == 1]) / len(y) #p1 ratio or classified vs total
        if p1 != 0 and p1 != 1: # For p1 = 0 and 1, set the entropy to 0 (handles 0 log0)
             entropy = -p1 * np.log2(p1) - (1 - p1) * np.log2(1 - p1) #entropy formula
        else:
             entropy = 0
    ### END CODE HERE ###        
